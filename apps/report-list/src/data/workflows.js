...
export const REPORT_TYPES = {
  'by-audience-attributes': 'By audience attributes',
  'by-conversation': 'By conversation',
  'by-upload': 'By upload',
  'by-account-profile': 'By account profile',
  'by-creator-discovery': 'By creator discovery',
  'by-popularity': 'By popularity',
  'by-overlap': 'By overlap',
  'by-custom-audience': 'By custom audience',
  'by-engagement': 'By engagement',
  'by-follower-growth': 'By follower growth',
  'by-fan-page': 'By fan page',
  'by-freeform-input': 'By freeform input',
}
...
/** Networks summarized by Multisource (`google`) for wizard tooltips. */
const MULTISOURCE_NETWORK_IDS_FOR_TOOLTIP = ['meta', 'x', 'linkedin', 'facebook', 'instagram', 'threads']

export function getMultisourceNetworksTooltipText() {
  const labels = MULTISOURCE_NETWORK_IDS_FOR_TOOLTIP.map((id) => getSourceLabel(id))
  if (labels.length === 0) return ''
  if (labels.length === 1) return `Includes ${labels[0]}.`
  const head = labels.slice(0, -1).join(', ')
  const last = labels[labels.length - 1]
  return `Includes ${head}, and ${last}.`
}
...
export function getWorkflowLabel(workflowId) {
  if (workflowId === 'all' || workflowId == null) return 'All'
  return WORKFLOW_BY_ID[workflowId]?.label ?? 'All'
}

/** Breadcrumb: reports list context (e.g. Home › Profile reports). */
export function getReportsListBreadcrumbLabel(workflowId) {
  if (workflowId === 'all' || workflowId == null) return 'All reports'
  return `${getWorkflowLabel(workflowId)} reports`
}
