...
import { BarChart3, Home, LayoutGrid, Menu, PieChart, UserSearch } from 'lucide-react'
import {
  TitanButton,
  TitanDialog,
  TitanNavbar,
...
/** Sidebar under "Workflows": same ids as `?workflow=` (all = union of every workflow). */
const WORKFLOWS_SIDEBAR_ITEMS = [
...
const NAV_ITEMS = WORKFLOWS_SIDEBAR_ITEMS
...
export default function AppLayout() {
  ...
  /** Mobile-only bottom drawer for the sidebar (≤900px). Hidden via CSS on desktop. */
  ...
  /** Wizard middle crumb: always the sidebar list you left (Workflows: All/Segment/Profile/Track or Search results), even for By source. */
  ...
  return (
    ...
    >
      ...
      <aside
        ...
        className={[
          'app-layout__sidebar',
          mobileNavOpen ? 'app-layout__sidebar--mobile-open' : '',
        ]
          .filter(Boolean)
          .join(' ')}
      >
        ...
        >
          ...
          <TitanSidebarSection>
            ...
            <TitanSidebarHeader>Workflows</TitanSidebarHeader>
            {WORKFLOWS_SIDEBAR_ITEMS.map((item) => {
              const Icon = item.icon
              return (
                <TitanSidebarItem key={item.id} id={item.id} icon={<Icon size={16} aria-hidden />}>
                  {item.label}
                </TitanSidebarItem>
              )
            })}
      ...
      <div className="app-layout__main">
        ...
        {!appsView ? <AppBreadcrumb items={breadcrumbItems} currentLabel={breadcrumbCurrentLabel} /> : null}
